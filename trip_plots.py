...
with mysql.connect(**config["webike.mysql"]) as mysql_client:
    ...
    fig2 = plt.figure()
    plt.hist([[start.hour for (start, end) in staff], [start.hour for (start, end) in students]], 24, normed=True, label=["staff/faculty", "students"])
    plt.xticks(range(0,24,2))
    plt.xlabel("hour of day")
    plt.ylabel("probability")
    plt.legend()
    ...
    plt.hist(data, bins=bins, zorder=2, rwidth=0.9, normed=True)
    plt.hist(data, bins=bins, cumulative=True, zorder=1, rwidth=0.9, normed=True)
    plt.xlabel("hour of day")
